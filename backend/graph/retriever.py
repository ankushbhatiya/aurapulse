...
def get_context_for_post(post_text: str, client_id: str = "CLIENT_A") -> str:
    ...
    try:
        with driver.session() as session:
            ...
            if not mentioned:
                logger.debug(f"No specific entities mentioned in: {post_text}. Using fallback context.")
        ...
        logger.error(f"Error fetching context from Neo4j: {e}")
        return "No historical context available due to error."
